...
import { saveUser, useQueryUserDetail } from "../../../../services/admins/users"
...
const UserDetailAdmin = () => {
    ...
    const { data: detailData, refetch } = useQueryUserDetail({ id }, { enabled: !!id })
    ...
    const dataTableShippingAddresses = shippingAddressFields.map((item, index) => {
        const fullAddress = [
            item.addressLine,
            item.ward,
            item.district,
    ...
    const onSubmit = handleSubmit((input) => {
        ...
        const updatedShippingAddresses = input.shippingAddresses?.map(addr => ({
            ...addr,
    ...
    useEffect(() => {
        if (detailData && id) {
            ...
            const defaultAddress = detailData?.shippingAddresses?.find(addr => addr.isDefault);
            if (defaultAddress) {
    ...
    return (
        <>
                ...
                <Grid templateColumns="repeat(12, 1fr)" gap={4} mb={6}>
                    {actionType === "modifiedAddresses" ? null :
                        <>  < GridItem colSpan={[6, 6, 4]}>
                            ...
                            {!isAdmin && <GridItem colSpan={[6, 6, 4]}>
                                <CoreInput
                                    control={control}
                                    name='password'
                                    label='Mật khẩu'
                    ...
                    <GridItem colSpan={12} mt={6}>
                        <Box display="flex" justifyContent="space-between" alignItems="center" mb={4}>
                            <Text fontSize="xl" fontWeight="bold">Địa chỉ giao hàng</Text>
                            <Button
                                colorScheme="blue"
                    ...
                    <GridItem colSpan={12} mt={6}>
                        <Box display="flex" justifyContent="center">
                            <Button
                                ...
                                type="submit"
                            >
                                {id ? 'Cập nhật thông tin' : 'Tạo người dùng mới'}
                            </Button>
                        </Box>
                    </GridItem>
                </Grid>
            ...
            < ShippingAddressModal isOpen={isOpenShippingModal}
                onClose={onCloseShippingModal}
                onSubmit={(data) => {
