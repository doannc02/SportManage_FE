...
const ShippingAddressModal = ({ isOpen, onClose, onSubmit }) => {
  const { data: infoCustomer } = useQueryInfoCurrentCustomer();
  const form = useForm({
    defaultValues: {
      recipientName: "",
      phone: "",
  ...
  const { data: dataAddressLv3, isLoading: isLoadingAddressLv3 } =
    useQueryAddressLv3(
      {
  ...
  const onSubmitAddress = form.handleSubmit(async (data) => {
    ...
    const selectedCity =
      dataAddressLv1?.data?.find((item) => item.id === data.cityId)
        ?.full_name || "";
    const selectedDistrict =
      dataAddressLv2?.data?.find((item) => item.id === data.district)
        ?.full_name || "";
    const selectedWard =
      dataAddressLv3?.data?.find((item) => item.id === data.ward)?.full_name ||
      "";
          ...
          <Grid templateColumns="repeat(12, 1fr)" gap={4}>
            ...
            <GridItem colSpan={12}>
              <FormControl display="flex" alignItems="center" mt={2}>
                <Checkbox colorScheme="blue" {...form.register("isDefault")}>
